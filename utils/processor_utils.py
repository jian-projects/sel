import random, torch, math, os, copy
...
def get_optimizer(args, model, methods=None):
    ...
    if 'AdamW_' in methods:
        ...
        for name, model_param in model.named_parameters():
            ...
            warmup_params.append({'params': model_param, 'lr': lr_ / 4 if id(model_param) in plm_params else lr_,
...
def get_metric(results, dataset):
    if dataset.task == 'cls':
        labels, preds, total_loss, total_loss_ce = [], [], 0, 0
        for rec in results:
            labels.extend(rec['labels'])
            preds.extend(rec['preds'])
            total_loss += rec['loss'].item() * len(rec['labels'])
            total_loss_ce += rec['loss_ce'].item() * len(rec['labels'])
        ...
        output['loss_ce'] = round(total_loss_ce / len(labels), 3)
...
class Metrics(object):
    def __init__(self, args, dataset):
        if dataset.task == 'cls':
            self.static = {
                'f1': 0, 'acc': 0, 'epoch': 0,
                'loss_bz': [], 'f1_bz': [], 'acc_bz': [],
                'loss_ep': [], 'f1_ep': [], 'acc_ep': []
            }
        self.train = copy.deepcopy(self.static)
        self.valid = copy.deepcopy(self.static)
        self.test = copy.deepcopy(self.static)
    ...
    def on_train_epoch_end(self, processor=None):
        self.train['f1_ep'].append(processor.best_result['train']['macro_f1'])
        self.train['acc_ep'].append(processor.best_result['train']['accuracy'])
    ...
    def on_train_start(self, processor=None):
        ...
        self.train['loss_ep'].append(rec_train['loss_ce'])
